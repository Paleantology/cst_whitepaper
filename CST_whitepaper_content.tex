...
\item [\textbf{TODO}]
    ...
    %\item Decide on pseudocode section (5.2) -- should it stay and be better integrated, be moved to a sub-section of 5.4, or removed? 
    %\rs{can we leave it for now - given the title change and overall expansion it might fit better now but let's see}
    %\rs{I'm ok with it now}
...
support students' job and/or graduate school preparation \citep{WilsonSayres18}. \cn{Are we changing all mentions of "student" to "learner"? (like previous sentence, for example) Want to make sure before I go through the intro and change the words.}
...
An alternate motivation is to incorporate more data and analysis \cn{into coursework? I think the wording of this sentence is a bit confusing. I think I know what it's saying, but not sure enough of the intended meaning to reword.},
...
\rs{add whatever Williams says are barriers} \citep{Williams17}.
...
\section{Programming Language or ??}
...
\begin{itemize}
    ...
    \item In this review, we will discuss methods for courses in which some degree of programming or computational data analysis is integrated
        \item In biology, R and Python are two dominant languages for working with datasets
        \item Because of their popularity, they make common choices for integration into coursework, and large ecosystems of tools have sprung up to support teaching with these languages
        \item All the tools discussed allow instructors to integrate programming adjacent skills, such as using a command line to run software via a UNIX or LINUX interface and using revision management to track changes to data and code
    \item There are many platforms that are outside of the scope of this review, such as Galaxy, a graphical user interface for genomics analysis available via the web
\end{itemize}
\aw{
I think whatever we want to do, we want to keep fairly brief
}
...
\rs{given this section move some stuff about R from the RStudio Server section}
...
%\rs{I'd agree with small classes, but for students with no prior experience I typically aim to avoid having them install anything as it's challenging}\aw{Perhaps 
students who leave the course self-sufficient to do computational work.
\rs{Do we want to recommend installing Anaconda, Jupyter, R, RStudio, etc. and explain how these enable package mangement?}
...
\rs{I had a case where the firewall interfered with logging into Amazon servers and IT had to specifically allow the server to be accessed. Also problematic if the server or host is down.}\aw{I had to call in the middle of class to have my github statis page for my course specifically whitelisted}
...
For instance, students could be encouraged or required to participate in discussions on the course website (e.g., Blackboard/Moodle) or a Slack workspace created for the class, or to comment on other students' GitHub projects. \rs{We haven't introduced Github here so people might not know it. Also, I know faculty have mixed success with discussion forums}
...
We have also discussed several teaching strategies for computational biology and other courses with computation, with the goal of improving student learning and retention.\cn{Sorry, I tried to stick with "learner" but I couldn't come up with another way to say "student learning"... feel free to suggest.} 
